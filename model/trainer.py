...
from model.gcn_cnn3 import GCNClassifier
...
def unpack_batch(batch, cuda):
    '''
    batch = words, masks, pos, ner, deprel, head, subj_positions, obj_positions, subj_type, obj_type, rels
    '''
    if cuda:
        inputs = [Variable(b.cuda()) for b in batch[:10]] #data[i:i+batch_size][:10]
...
class GCNTrainer(Trainer):
    ...
    def update(self, batch):
        ...
        #labels = [0,1,3,4,...,41,0] labels中为batch_size条数据对应的关系的id
        #logits = [b,42]
    ...
    def predict(self, batch, unsort=True):
        ...
        loss = self.criterion(logits, labels)
        probs = F.softmax(logits, 1).data.cpu().numpy().tolist() #[b,42]
        predictions = np.argmax(logits.data.cpu().numpy(), axis=1).tolist() #按行取最大的下标[b,1]
        if unsort:
